...
module RunningProgram =
    let makeCopy p =
        { p with
            Mem = Array.copy p.Mem
            Input = Queue<int64>(p.Input)}
...
let rec execute (p : RunningProgram) (ctk : CancellationToken option) =
    ctk |> Option.iter (fun ctk -> ctk.ThrowIfCancellationRequested())
    ...
    | Add _ ->
        ...
        execute { p with Pos = p.Pos + (Opcode.instrMemCount opCode) } ctk
    ...
    | JumpIfTrue _  ->
        ...
        execute { p with Pos = nextPos } ctk
    ...
    | AdjustRelativeBase _ ->
        ...
        execute { p with
                    ...
                    RelBase = p.RelBase + int inAVal } ctk
...
let rec executeUntilHaltOrTimeout (p : RunningProgram) timeoutMs =
    let rec executeUntilHalt' (p : RunningProgram) acc ctk =
        match execute p (Some ctk) with
        | Terminate -> acc, Terminated
        | SuspendUntilNextInput p -> acc, SuspendedWaitingInput p
        | PerformOutput (o, p) -> executeUntilHalt' p (o::acc) ctk

    let executeAsync = async {
        let! ctk = Async.CancellationToken
        ctk.ThrowIfCancellationRequested ()
        let output, p = executeUntilHalt' p [] ctk
        return List.rev output, p
    }

    try
        Async.RunSynchronously(executeAsync, timeoutMs)
    with
